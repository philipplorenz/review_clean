...
data<- read_excel("data_review.xlsx")
...
######################################################
# outcome and digital media variable matrix

combinations_simple <- data %>%
  dplyr::count(outcome_clean, dm_clean) %>%
  #filter(!(method_clean=="survey" & outcome_clean=="participation"))%>%
  na.omit()%>%
  arrange(desc(n))
...
p <- ggplot(combinations_simple, aes(x = reorder(dm_clean, -n), y = outcome_clean, fill = n)) +
  geom_tile() +
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 12),
        axis.text.y = element_text(size = 12),
        ...
        axis.title.x = element_text(size = 15))+
  scale_y_discrete(limits=o) +
  scale_fill_viridis_c(direction = -1, trans = "log", breaks=my_breaks, na.value="white", name = "Number of Measurements", option = "inferno")
